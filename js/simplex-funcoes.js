...
function criarFormMax(p_variaveis, p_restricoes, p_interacoes) {
	console.log(p_interacoes);
	...
	if (p_variaveis == "" || p_variaveis <= 0 || p_variaveis != parseInt(p_variaveis)) {
		...
		if (p_restricoes == "" || p_restricoes <= 0 || p_restricoes != parseInt(p_restricoes)) {
			...
			form1.regras.focus();
			return;
		} if (p_interacoes == "" || p_interacoes <= 0 || p_interacoes != parseInt(p_interacoes)) {
			alert('Especifique a quantidade de interações.');
			form1.interacoes.focus();
			return;
		}
	}
...
function criarFormMin(p_variaveis, p_restricoes) {
...
function resolver() {
	...
	// Adicionando linhas com as variavéis de folga. Ex: 'f1', 'f2'
	...
	for (var n = 1; n <= variaveis; n++) {
		...
		if (numFormatado >= 0){
			console.log(numFormatado);
			document.getElementById("tab").innerHTML+="<p><b>"+"Solução Ótima"+"</b></p>";
		} else if (numFormatado < 0) {
			console.log(numFormatado);
			document.getElementById("tab").innerHTML+="<p><b>"+"Solução Ilimitada"+"</b></p>";
		}
